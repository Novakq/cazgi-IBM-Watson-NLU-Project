    ...
    sendForSentimentAnalysis = () => {
        ...
        fetch(url).then((response) => {
            response.json().then((data) => {
                this.setState({ sentimentOutput: data.document.label });
               // let output = data;
                let color = "black";

                switch (data.document.label) {
                    case "positive": color = "green"; break;
                    case "negative": color = "red"; break;
                    default: color = "yellow";
                }
               // output = <div style={{ 'color':  color, fontSize: 20 }}>{data}</div>
                this.setState({ sentimentOutput: <SentimentTable sentiment={data} semanticColor={color} /> });
            })
        });
    }
